...
type Props = {
  ...
  /**
   * No telemóvel, o vídeo só aparece antes do catálogo filtrado.
   * Em `md+` o vídeo continua sempre visível.
   */
        ...
        {rawUrl && ytId ? (
          <div className="relative aspect-video w-full bg-black">
            <iframe
              ...
              allowFullScreen
              // Eager: evita o iframe sumir/ficar em branco após o catálogo carregar no mobile
              loading="eager"
...
export function CategoryShowcaseBanner({
  ...
  return (
    <section className="mb-6 flex flex-col gap-3 md:grid md:grid-cols-2 md:items-start">
        ...
        <div className="mt-3 overflow-hidden rounded-lg border border-white/[0.08]">
          <table className="w-full border-collapse text-sm">
            <thead className="bg-white/[0.04]">
              <tr>
                <th className="px-3 py-2 text-left font-medium text-stone-300">
                  Quantidade
                </th>
                <th className="px-3 py-2 text-left font-medium text-stone-300">
                  Valor por peça
      ...
      >
        <QualityVideo categoryLabel={categoryLabel} config={config} />
      </div>
